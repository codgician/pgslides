...
其实本质上就是等价类…… 部分地方记作 $\mathcal{O}_G(x)$。
...
为群 $G$ 作用下 $x$ 的**稳定化子 (stabilizer)**。

::: notes

又称固定子群，有的地方也写成 $G_m$ 或 $G^m$。
...
- 记用于染色的颜色集合为 $C$， $c(g)$ 为置换 $g$ 被分解为不交轮换乘积的个数，则：
...
- 记 $dp\langle i, j, k, l \rangle$ 代表 $i$ 个水晶排成一排，最后三个元素的颜色分别为 $j, k, l$，则方案数：
  ...
  $$
- 枚举前三个水晶的颜色 $\langle j, k, l \rangle$，则 $dp \langle i + 3, j, k, l \rangle$ 代表围成一圈时满足题目条件的不动元个数。
